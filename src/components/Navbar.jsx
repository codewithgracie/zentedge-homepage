    ...
    <div className='Absolute top-0 left-o w-full z-10'>
        <div className='container mx-auto flex justify-between items-center py-5 px-6 md:px-20 lg:px-32 bg-transparent' >
            ...
            <ul className='hidden md:flex gap-7 text-white'>
                ...
                <a href="#Header" className='cursor-pointer hover:text-green-400'>About</a>
        ...
        <div className={`md:hidden ${showMobileMenu ? 'fixed w-full' : 'h-0 w-0'}fixed w-full right-0 top-0 bottom-0 overflow-hidden bg-white transition-all`}> 
            <div className='flex justify-end p-5 cursor-pointer'>
                <img onClick={() => setShowMobileMenu(false)} src={Close} alt="close icon" className='w-6' />
            </div>
            <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
                <a onClick={() => setShowMobileMenu(false)} href="" className='px-4 py-2 rounded-full inline'>Home</a>
                <a onClick={() => setShowMobileMenu(false)} href="" className='px-4 py-2 rounded-full inline'>About</a>
                <a onClick={() => setShowMobileMenu(false)} href="" className='px-4 py-2 rounded-full inline'>Services</a>
                <a onClick={() => setShowMobileMenu(false)} href="" className='px-4 py-2 rounded-full inline'>Pricing</a>
                <a onClick={() => setShowMobileMenu(false)} href="" className='px-4 py-2 rounded-full inline'>Projects</a>
                <a onClick={() => setShowMobileMenu(false)} href="" className='px-4 py-2 rounded-full inline'>Contact</a>
            </ul>
        </div>
    </div>
  )
